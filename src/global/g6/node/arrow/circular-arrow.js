...
export default {
  ...
  options: {
    ...
    getShapeStyle (cfg) {
      ...
      // 左下左
      let P1 = { x: -width / 2, y: height / 2 - L1 }
      // 左下右
      let P2 = { x: -width / 2 + 2 * L7, y: height / 2 - L1 }
      const path = [
        [ 'M', P1.x, P1.y ],
        // FIXME 圆弧的中心点不在坐标原点上
        [ 'A', (width - (L2 - L7)) / 2, height - L1, 0, 1, 1, A0[1].x, A0[1].y ],
        [ 'L', A0[2].x, A0[2].y ],
        [ 'L', A0[3].x, A0[3].y ],
        [ 'L', A0[4].x, A0[4].y ],
        [ 'L', A0[5].x, A0[5].y ],
        // FIXME 圆弧的中心点不在坐标原点上
        [ 'A', (width - (L2 - L7)) / 2 - 2 * L7, height - L1 - 2 * L7, 0, 1, 0, P2.x, P2.y ],
        [ 'Z' ]
      ]
